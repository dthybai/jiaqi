<template>
	<view :class="titlIndex === 1 ? 'page-gray' : ''">
		...
		<!-- 视频组件 -->
		<view style="padding: 8rpx 40rpx 0;" class="boxs" v-show="titlIndex === 0">
...
<script>
	...
	export default {
		...
		methods: {
			...
			gethistorylist(type,pageNum,pageSize){
				this.$api.historylist({type:type,page:pageNum,size:pageSize}).then(res => {
				   // 获得历史下得视频数据 
				   console.log(res)
				  if(res.code === 200){	
						 if(this.type===1){
			...
			desucceed(){ // 确认删除
				...
				console.log(goodsid)
				this.$api.historydelete({ids:goodsid}).then(res => {
				  if(res.code === 200){	
					  console.log(res)
					   this.payDialogVisible = false;
					   this.allAdminCheckState = false;
			...
			searchHandler (pageNum,pageSize) {
				if(this.titlIndex === 0) {
					this.type = 1;
					this.gethistorylist(this.type,pageNum,pageSize)
